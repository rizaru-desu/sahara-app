...
import { pageAllAgent, pageAllUser } from "@/app/utils/db/controllerDB";
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (tokenValidated) {
      ...
      const { detail, allAgent, totalAgent } = await pageAllAgent({
        skip: resultValid.skip,
        take: resultValid.take,
        userId: userId,
      });

      const finalResult = _.map(allAgent, (item) => {
      ...
      return NextResponse.json(
        {
          allAgent: finalResult,
          countAgent: totalAgent,
          userDetail: detail,
        },
        {
          status: 200,
