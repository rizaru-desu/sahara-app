...
import { pageAllAgentRedeem } from "@/app/utils/db/controllerDB";
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (tokenValidated) {
      ...
      const { detail, allRedeem, totalRedeem, statusMap } =
        await pageAllAgentRedeem({
          skip: resultValid.skip,
          take: resultValid.take,
          userId: userId,
        });

      const mappedData = _.map(allRedeem, (d) => {
