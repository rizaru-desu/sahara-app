...
import { batchProduct } from "@/app/utils/db/productDB";
import z from "zod";
...
export async function POST(request: NextRequest) {
  try {
    ...
    validateSchema({
      data: { format: files[0].type, createBy },
    });
    ...
    if (tokenValidated) {
      ...
      const result = await batchProduct({ dataProduct: newArray as [] });

      return NextResponse.json(
        {
          message: "Upload completed",
          data: `Successfully insert: ${result.count}`,
        },
        {
          status: 200,
        }
