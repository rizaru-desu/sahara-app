...
import { pageAllOwnerBooth } from "@/app/utils/db/controllerDB";
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (tokenValidated) {
      ...
      const { detail, allBoothOwner, totalBoothOwner } =
        await pageAllOwnerBooth({
          skip: resultValid.skip,
          take: resultValid.take,
          userId: userId,
        });

      const finalResult = _.map(allBoothOwner, (item) => {
        return Object.assign(
          {
            id: item.boothOwnerId,
          },
          _.omit(item, "boothOwnerId")
        );
      });

      return NextResponse.json(
        {
          allBoothOwner: finalResult,
          countBoothOwner: totalBoothOwner,
          userDetail: detail,
        },
        {
          status: 200,
