...
import { pageAllLoyalty } from "@/app/utils/db/controllerDB";
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (tokenValidated) {
      ...
      const { detail, allLoyalty, totalLoyalty } = await pageAllLoyalty({
        skip: resultValid.skip,
        take: resultValid.take,
        userId: userId,
      });

      const transformedData = _.map(allLoyalty, (obj) => {
        const userWithoutData = _.omit(obj, "userIdData");
        return _.assign({}, userWithoutData, obj.userIdData);
      });

      return NextResponse.json(
        {
          allLoyalty: transformedData,
          userDetail: detail,
          countLoyalty: totalLoyalty,
        },
        {
          status: 200,
        }
