...
export async function POST(request: NextRequest) {
  try {
    ...
    if (tokenValidated) {
      ...
      const { detail, allPackage, totalPackage } = await pageAllPackageRedeem({
        skip: resultValid.skip,
        take: resultValid.take,
        userId: userId,
      });

      const finalResult = _.map(allPackage, (item) => {
        return Object.assign(
          {
            photo: `${baseURL}/api/controller/packageRedeem/image/${item.packageId}`,
          },
          _.omit(item, "photo")
        );
      });
      ...
      return NextResponse.json(
        {
          allPackage: finalResult,
          countPackage: totalPackage,
          userDetail: detail,
        },
        {
          status: 200,
